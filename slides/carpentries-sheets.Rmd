...
caption <- "Count of female (F) and male (M) individuals in each plot."
knitr::kable(wide, caption = caption)
```

## From wide to long, automatically | The key workd is automatically. Example in R:
...
Format dates so that any program can interpret the dates correctly.
...
Cell range may be define in another sheet
